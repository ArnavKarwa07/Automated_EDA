...
def create_access_token(user_id: str, email: Optional[str] = None, expires_delta: Optional[timedelta] = None):
...
async def get_optional_user(credentials: Optional[HTTPAuthCredentials] = Depends(security)) -> Optional[TokenData]:
...
class AuthService:
    ...
    @staticmethod
    async def signup(email: str, password: str, username: Optional[str] = None, full_name: Optional[str] = None):
        """Register a new user"""
        from services.database_manager import db_manager
    ...
    def refresh_token(current_user: TokenData) -> Token:
        ...
        new_token = create_access_token(user_id=current_user.user_id, email=current_user.email)
        return Token(
