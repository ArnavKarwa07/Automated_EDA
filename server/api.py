...
async def auto_generate_dashboard(
    ...
    try:
        ...
        result = await dashboard_interface.auto_generate_dashboard(file_path, business_context or "")
...
async def generate_interactive_dashboard(
    ...
    """Generate an AI-powered interactive dashboard using Groq"""
    if file_id not in uploaded_files:
        raise HTTPException(status_code=404, detail="File not found")
    ...
    try:
        ...
        if include_raw_data:
            ...
            for col in summary_stats['categorical_columns'][:5]:  # Limit to 5 categorical columns
                unique_vals = df[col].dropna().unique()[:20].tolist()  # Limit to 20 unique values
        ...
        return {
            ...
            "dashboard": {
                ...
                "metadata": convert_numpy_types({
                    ...
                    "interactive_features": ["ai_insights", "filters", "kpis", "drill_down"],
        ...
        raise HTTPException(status_code=500, detail=f"Interactive dashboard generation failed: {str(e)}")

@router.post("/dashboard/generate")
async def generate_dashboard(
    file_id: str = Form(...),
    dashboard_type: str = Form("auto"),  # auto, executive_summary, data_quality, exploratory
    ...
    try:
        ...
        requirements = await dashboard_builder.analyze_dashboard_requirements(df, dashboard_type)
        
        # Generate dashboard
        dashboard = await dashboard_builder.generate_dashboard(df, requirements, custom_options)
...
async def get_dashboard_recommendations(
    ...
    try:
        dashboard_interface = DashboardMCPInterface()
        result = await dashboard_interface.get_dashboard_recommendations(file_path, business_context or "")
